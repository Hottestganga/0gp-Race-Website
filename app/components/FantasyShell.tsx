    ...
    <nav className="sticky top-0 z-50 border-b border-yellow-500/20 bg-black/90 backdrop-blur-xl">
      <div className="site-center-wide flex items-center justify-between gap-5 px-4 py-3 sm:px-7">
        ...
        <div className="flex items-center gap-3">
            ...
            {links.map(([href, label, key]) => (
              <Link
                ...
                className={`whitespace-nowrap transition ${
                  active === key ? "text-yellow-300" : "text-zinc-500 hover:text-white"
                }`}
              >
                {label}
              </Link>
          ...
          <a
            href="#"
            title="Discord invite link will be added later"
...
function Embers() {
  const specs = [
    ["12%", "20%", "0s"], ["22%", "72%", "1.2s"], ["78%", "18%", "2.6s"],
    ["88%", "65%", ".6s"], ["65%", "88%", "3.5s"], ["35%", "84%", "2s"],
  ];
...
export function SectionTitle({
  ...
  return (
    <div>
      ...
      {text && <p className="mt-5 max-w-3xl text-base leading-7 text-zinc-400">{text}</p>}
    </div>
  );
}
